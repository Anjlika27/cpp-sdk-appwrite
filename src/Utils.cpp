...
// Helper functions (now as static members)
static void setCurlPathForWindows(CURL *curl) {
#ifdef _WIN32
...
int Utils::postRequest(const std::string &url, const std::string &payload,
...
int Utils::getRequest(const std::string &url, const std::vector<std::string> &headers,
                     std::string &response) {
    CURL *curl = curl_easy_init();
    if (!curl) {
        std::cerr << "cURL initialization failed!" << std::endl;
...
int Utils::putRequest(const std::string &url, const std::string &payload,
...
int Utils::patchRequest(const std::string &url, const std::string &payload,
                       const std::vector<std::string> &headers,
                       std::string &response) {
    CURL *curl = curl_easy_init();
    if (!curl) {
...
int Utils::deleteRequest(const std::string &url,
                        const std::vector<std::string> &headers,
                        std::string &response) {
    CURL *curl = curl_easy_init();
    if (!curl) {
